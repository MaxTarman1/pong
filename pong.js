...
function update(){
    ...
    context.fillStyle = 'skyblue';
    let nextPosition = player1.y+player1.velocityY;
    if(!outOfBounds(nextPosition)) player1.y=nextPosition;

    //player1.y += player1.velocityY;
    context.fillRect(player1.x,player1.y,player1.width,player1.height);
    nextPosition = player2.y+player2.velocityY;
    if(!outOfBounds(nextPosition)) player2.y=nextPosition;
    ...
    if(detectCollision(ball,player2)){
        if(ball.x+ball.width>= player2.x) ball.velocityX*=-1;
    }
    if(ball.x < 0){
        player2Score+=1;
        resetGame(1);
